...
List get_gene_expression_level(NumericVector counts, NumericVector cell_size,
    ...
    for (int k = 0; k < numbin; ++k) {
        ...
        // using eq 27
        q_v[k] = fitfrac(f.data(), q, counts.begin(), C, gene_size, var_g[k], cell_size.begin(), b);

        /*** Compute optimal delta given v_g ***/
        for (int c = 0; c < C; ++c) {
            delta_v[k][c] = var_g[k] * (counts[c] - f[c] * gene_size); // eq 28 in supp rearranged to avoid logs
        }
